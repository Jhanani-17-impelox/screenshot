...
#     """A Text widget with basic Markdown rendering capabilities"""
...
#                 if bold_match and (not italic_match or bold_match.start() < italic_match.start()) and (not link_match or bold_match.start() < link_match.start()):
#                     # Insert text before bold
#                     self.insert(tk.END, line_remaining[:bold_match.start()])
#                     # Insert bold text
#                     self.insert(tk.END, bold_match.group(1), "bold")
#                     # Update remaining line
#                     line_remaining = line_remaining[bold_match.end():]
#                 elif italic_match and (not bold_match or italic_match.start() < bold_match.start()) and (not link_match or italic_match.start() < link_match.start()):
#                     # Insert text before italic
#                     self.insert(tk.END, line_remaining[:italic_match.start()])
#                     # Insert italic text
#                     self.insert(tk.END, italic_match.group(1), "italic")
#                     # Update remaining line
#                     line_remaining = line_remaining[italic_match.end():]
#                 elif link_match and (not bold_match or link_match.start() < bold_match.start()) and (not italic_match or link_match.start() < italic_match.start()):
#                     # Insert text before link
#                     self.insert(tk.END, line_remaining[:link_match.start()])
#                     # Insert link text
#                     self.insert(tk.END, link_match.group(1), "link")
#                     # Update remaining line
#                     line_remaining = line_remaining[link_match.end():]
#                 else:
...
#         self.button_window.bind("<ButtonPress-1>", self.start_move)
#         self.button_window.bind("<ButtonRelease-1>", self.stop_move)
#         self.button_window.bind("<B1-Motion>", self.do_move)

#     def toggle_move_mode(self):
#         self.move_mode = not self.move_mode
#         if self.move_mode:
#             # Visual indicator that we're in move mode
#             self.move_button.configure(bg=self.colors["error"], text="⋮⋮")
#             self.update_status("Move mode ON - drag to reposition button", "info")
#         else:
#             self.move_button.configure(bg=self.colors["primary"], text="⋮⋮")
#             self.update_status("Move mode OFF - click to capture", "info")
...
#         # Don't do anything special on release
#         self.x = None
#         self.y = None
